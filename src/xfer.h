...
inline bool xferCommand(JsonDocument& doc) {
  ...
  if (strcmp(cmd, "status") == 0) {
    ...
    // M5PM1 PMIC on StickS3 has no battery-current register. Use a
    // signed sentinel for mA: -1 = charging, +1 = discharging. The
    // desktop only reads sign for the charging indicator. REFERENCE.md
    // lets us omit fields we don't have, but keeping mA present keeps
    // the JSON shape stable for the desktop status panel.
