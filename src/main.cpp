...
const int LED_PIN = 19;          // red LED, active-low (StickS3 GPIO19, shared with USB D+)
...
bool    menuOpen    = false;
uint8_t menuSel     = 0;
uint8_t brightLevel = 2;           // 0..4 → brightness 60..252; default 2 for longer runtime on the S3's larger backlight
bool    btnALong    = false;
...
// brightLevel 0..4 → M5Unified brightness 60..252 (matches StickCPlus
// ScreenBreath 20..100 mapped to the 0..255 range).
static void applyBrightness() { M5.Display.setBrightness(60 + brightLevel * 48); }
...
static void clockRefreshRtc() {
  ...
  _clkLastRead = millis();
  _onUsb = (M5.Power.isCharging() == m5::Power_Class::is_charging);
...
void drawInfo() {
  ...
  if (infoPage == 0) {
    ...
    // M5PM1 PMIC on StickS3 has no current/VBUS readings. Use charging
    // state (digital) as the proxy; report mA=0 on the page.
    int vBat_mV = M5.Power.getBatteryVoltage();   // M5Unified returns mV
    int iBat_mA = 0;                              // not measurable on M5PM1
    bool usb = (M5.Power.isCharging() == m5::Power_Class::is_charging);
    int vBus_mV = usb ? 5000 : 0;                 // nominal — no real read
    ...
    ln("  temp     %dC", (int)temperatureRead());   // ESP32-S3 internal, ±10C
...
void setup() {
  Serial.begin(115200);
  delay(50);
  // M5Unified auto-configures board (m5stack-sticks3): IMU (BMI270),
  // display, buttons, PMIC (M5PM1), speaker. No hardware RTC on StickS3.
  auto cfg = M5.config();
...
void loop() {
  ...
  // 6s hard-shuts down via PMIC hardware (no software handling needed).
  // Note: on StickS3/M5PM1, M5.Power.getKeyState() is a no-op — we read
  // the digital button line via M5.BtnPWR.
  ...
  if (!napping && faceDownFrames >= 15) {
    napping = true;
    napStartMs = now;
    M5.Display.setBrightness(30);   // dim but still visible — matches old ScreenBreath(8)
    dimmed = true;
    setCpuFrequencyMhz(80);         // nap = nothing to render, drop clock for power
  ...
  // When the screen is off the loop has nothing urgent to do — trade the
  // 100 ms busy-wait for a timer-gated light sleep. The BT controller
  // handles its own modem sleep around this, so an active BLE link stays
  // up; wake latency on timer expiry is a few ms.
  if (screenOff) {
